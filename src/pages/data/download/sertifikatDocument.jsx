...
const SertifikatDocument = () => {
  ...
  const breadcrumbs = ["Data Table", "Download", "Setifikat Dokumen"];

  useEffect(() => {
    const fetchDatas = async () => {
      try {
        const { data } = await axios.get(
          "https://knowledgeable-painted-guarantee.glitch.me/sertifikat_document"
        );
        setDatas(data);
      } catch (error) {
        console.log(error);
      ...
      <Flex className="h-fit flex-col gap-3">
        <Box className="bg-secondaryGray-300 rounded-xl py-5 px-10">
          <h2 className="flex items-center gap-2 font-semibold text-xl">
            <FaCertificate className="text-brandTabs-300" /> Serifikat Documen
          </h2>
        </Box>
