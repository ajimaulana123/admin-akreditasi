...
const SertifikatDosen = () => {
  const [datas, setDatas] = useState(null);
  const breadcrumbs = ["Data Table", "Download", "Sertifikat Dosen"];

  useEffect(() => {
    const fetchDatas = async () => {
      try {
        const { data } = await axios.get(
          "https://knowledgeable-painted-guarantee.glitch.me/sertifikat_dosen"
        );
        setDatas(data);
      } catch (error) {
        console.log(error);
      ...
      <Flex className="h-fit flex-col gap-3">
        <Box className="bg-secondaryGray-300 rounded-xl py-5 px-10">
          <h2 className="flex items-center gap-2 font-semibold text-xl">
            <FaCertificate className="text-brandTabs-300" /> Sertifikat Dosen
          </h2>
        </Box>
...
export default SertifikatDosen;
